...
function account(url) {
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get('https://lampac-ukraine.mooo.com/player-inner/' + element.url);
      return;
    }
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      onContextLauch: function onContextLauch(object) {
        ...
        Lampa.Activity.push({
          url: '',
          title: Lampa.Lang.translate('title_online_shara'),
          component: 'SHARA',
    ...
    var svg = '<svg width="800px" height="800px" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" class="a" d="M29.3323,25.9072a2.2,2.2,0,0,0,0-3.8144l-5.3172-3.0592,5.4833-2.5569,2.3028,1.3248L39.26,22.0928a2.2,2.2,0,0,1,0,3.8144l-7.4561,4.29-.0017.0014-2.4851,1.43-5.4833-2.5569ZM12.5878,29.29v2.4473a2.2,2.2,0,0,0,3.2965,1.9069l.0009-.0006,2.7034-1.5553,5.4832,2.5569-5.7177,3.29-.0037.0023-7.4157,4.2666a2.2,2.2,0,0,1-3.2976-1.9074V26.9815ZM24.2543,13.46,18.771,16.0165l-2.8856-1.66-.0017-.0009a2.2,2.2,0,0,0-3.2958,1.9082V18.9L7.6371,21.2083V7.7036a2.2,2.2,0,0,1,3.2976-1.9072l7.4186,4.2682.0009,0Z"/></svg>';
    var button = "<div class=\"full-start__button selector view--online shara--button\" data-subtitle=\"".concat(manifst.name, " ").concat(manifst.version, "\">\n" + svg + "\n\n        #{title_online_shara}\n    </div>"); // нужна заглушка, а то при страте лампы говорит пусто
    Lampa.Component.add('SHARA', component); //то же самое
    resetTemplates();

    function addButton(e) {
      if (e.render.find('.shara--button').length) return;
    ...
    Lampa.Listener.follow('full', function(e) {
      if (e.type == 'complite') {
        ...
        addButton({
          render: playbtn.length > 0 ? playbtn : e.object.activity.render().find('.view--torrent'),
          movie: e.data.movie
        });
      }
    });
  ...
  if (!window.SHARA_plugin) startPlugin();
